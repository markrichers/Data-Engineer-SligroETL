...
def insert_data(cur, row):
    # Check if this row already exists in the table
    cur.execute("SELECT COUNT(*) FROM event_sessions WHERE customer_id=%s AND event_type=%s AND timestamp=%s", tuple(row))
    if cur.fetchone()[0] > 0:
        # If it does, skip it the count is greater than 0
        return
    # Build the insert query with %s is a placeholder for the values
    query = "INSERT INTO event_sessions (customer_id, event_type, timestamp) VALUES (%s, %s, %s)"
    # Execute the query
    try:
        cur.execute(query, tuple(row))
        print(f"Successfully inserted row {row.name + 1} into the database.")
    except (Exception, psycopg2.DatabaseError) as error:
        print(f"Error occurred while inserting row {row.name + 1} into the database: {error}")
...
def processing_data():
    ...
    postgres_pool = psycopg2.pool.SimpleConnectionPool(1, 10, user = DB_USER,
    ...
    cur.execute("SELECT EXISTS (SELECT FROM information_schema.tables WHERE table_name='event_sessions')")
    exists = cur.fetchone()[0]

    # If it exists, print the current number of rows
    if exists:
        cur.execute("SELECT COUNT(*) FROM event_sessions")
        count = cur.fetchone()[0]
        print(f"Existing rows in the 'event_sessions' table before processing: {count}")

    # If it doesn't exist, create it
    if not exists:
        create_table_query = '''CREATE TABLE event_sessions (
                                customer_id INTEGER,
    ...
    # Use a ThreadPoolExecutor for the database insertions
    with concurrent.futures.ThreadPoolExecutor() as executor:
        # The map function applies the insert_data function to every item in df.iterrows()
        executor.map(lambda row: insert_data(cur, row), df.iterrows())

    # Calculate processing time
    end_time = time.time()
    total_time = end_time - start_time
    print(f"Total processing time: {total_time / 60} minutes")
